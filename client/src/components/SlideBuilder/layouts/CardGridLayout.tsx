...
export function CardGridLayout(props: SlideLayoutProps) {
  const { contentBlocks, primaryColor, rtl, dimensions } = props;
  const blockCount = contentBlocks.length;
  const gridCols = blockCount > 12 ? 'grid-cols-2 sm:grid-cols-3 md:grid-cols-4' : blockCount > 6 ? 'grid-cols-2 sm:grid-cols-3' : 'grid-cols-2';
